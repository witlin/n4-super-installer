...
if (Test-Path -Path $ctrlDir) {
  ...
  log-step $infoBase" D:\Controls Software does not exist..."

  if (Test-Path -Path "D:\") {
    
      ### D:\ volume exists on disk, create the Controls Software folder.
      New-Item -Path "D:\" -Name "Controls Software" -ItemType "directory"
      ...
      <# NOT ENOUGH DISK SPACE IN THE TEST VM !!!!
